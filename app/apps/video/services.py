...
async def process_result(video: Video):
    result = await fal_client.result_async(f"fal-ai{video.engine.get_bot_url}", video.request_id)
...
async def get_fal_status(video: Video):
    status = await fal_client.status_async(f"fal-ai{video.engine.get_bot_url}", video.request_id, with_logs=True)
